...
_**Тестовая часть инструкции, в будущем будет удалена!**_

Создали новую ветку для слияния без конфликта

Создаем коммит

Сохраняем master после слияния веток

Оставили свободное место 136-148 для слияния веток без конфликта.
Перенесли текст на несколько строк ниже.

Попытка номер два, так как не получилась графа слияния в --graph. Думаю что из-за пропущенного коммита в master, пробуем.

Тут немного добавим информации чтоб ветка была чуть длиннее, погорячился с названием коммита, он был не последний :D

----

Добавляем инфу в тест